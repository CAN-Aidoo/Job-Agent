...
import { ApplicationDraft, DraftStatus } from '../interfaces/job';
...
interface DraftRow {
  ...
  screening_answers: unknown | null;
  status: DraftStatus;
...
export async function findById(id: string): Promise<DraftRow | null> {
  const pool = getPool();
  const { rows } = await pool.query<DraftRow>(
    'SELECT * FROM application_drafts WHERE id = $1',
    [id]
  );
  return rows[0] || null;
}
...
export async function update(id: string, partial: Partial<DraftRow>): Promise<DraftRow | null> {
  ...
  if (partial.cover_letter !== undefined) { sets.push(`cover_letter = $${idx++}`); values.push(partial.cover_letter); }
  if (partial.screening_answers !== undefined) { sets.push(`screening_answers = $${idx++}`); values.push(JSON.stringify(partial.screening_answers)); }
  if (partial.resume_variant_id !== undefined) { sets.push(`resume_variant_id = $${idx++}`); values.push(partial.resume_variant_id); }
  if (partial.user_feedback !== undefined) { sets.push(`user_feedback = $${idx++}`); values.push(partial.user_feedback); }
  if (partial.approved_at !== undefined) { sets.push(`approved_at = $${idx++}`); values.push(partial.approved_at); }
  if (partial.submitted_at !== undefined) { sets.push(`submitted_at = $${idx++}`); values.push(partial.submitted_at); }
  if (partial.submission_receipt !== undefined) { sets.push(`submission_receipt = $${idx++}`); values.push(JSON.stringify(partial.submission_receipt)); }
