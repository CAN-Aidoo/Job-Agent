...
router.get('/overview', async (req: AuthRequest, res: Response) => {
  ...
  const [totalDrafted, totalSubmitted, totalPending, avgScore, topCompanies, weeklyApps, outcomes] = await Promise.all([
    pool.query<{ count: number }>('SELECT COUNT(*)::int as count FROM application_drafts WHERE user_id = $1', [userId]),
    pool.query<{ count: number }>(
      `SELECT COUNT(*)::int as count FROM application_drafts WHERE user_id = $1 AND status = 'submitted'`,
      [userId],
    ),
    pool.query<{ count: number }>(
      `SELECT COUNT(*)::int as count FROM application_drafts WHERE user_id = $1 AND status IN ('pending_review', 'awaiting_approval')`,
      [userId],
    ),
    pool.query<{ avg: number }>(
      'SELECT COALESCE(AVG(match_score), 0)::float as avg FROM application_drafts WHERE user_id = $1',
      [userId],
    ),
